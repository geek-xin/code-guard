import { useState } from 'react';
import { Shield, LayoutDashboard, FolderGit2, ScanSearch, Database, LogOut, Github } from 'lucide-react';
import { api, setToken, SessionUser } from '@/lib/api';
import { cn } from '@/lib/utils';
import { useEffect } from 'react';

const NAV = [
  { key: 'dashboard', label: '总览', icon: LayoutDashboard, href: '#/dashboard' },
  { key: 'projects', label: '项目', icon: FolderGit2, href: '#/projects' },
  { key: 'scans', label: '扫描记录', icon: ScanSearch, href: '#/scans' },
  { key: 'vulndb', label: '漏洞库', icon: Database, href: '#/vulndb' },
];

export default function Layout({ children, current }: { children: React.ReactNode; current: string }) {
  const [user, setUser] = useState<SessionUser | null>(null);

  useEffect(() => {
    api.me().then(setUser).catch(() => {});
  }, []);

  return (
    <div className="flex min-h-screen">
      <aside className="fixed inset-y-0 left-0 z-30 flex w-60 flex-col border-r-chunky border-ink bg-white">
        <div className="flex items-center gap-3 border-b-2 border-ink px-5 py-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-md border-chunky border-ink bg-primary shadow-chunky-sm">
            <Shield className="h-6 w-6 text-white" strokeWidth={2.5} />
          </div>
          <div>
            <div className="text-base font-black leading-tight text-ink">CodeGuard</div>
            <div className="text-[11px] font-semibold text-ink-muted">代码安全分析平台</div>
          </div>
        </div>
        <nav className="flex-1 space-y-1.5 px-3 py-4">
          {NAV.map((item) => {
            const Icon = item.icon;
            const active = current === item.key || (item.key === 'projects' && current === 'scans');
            return (
              <a
                key={item.key}
                href={item.href}
                className={cn(
                  'flex items-center gap-2.5 rounded-md border-chunky px-3 py-2.5 text-sm font-bold transition-all',
                  active
                    ? 'border-ink bg-paper-alt text-ink shadow-chunky-sm'
                    : 'border-transparent text-ink-muted hover:bg-paper hover:text-ink',
                )}
              >
                <Icon className="h-4 w-4" />
                {item.label}
              </a>
            );
          })}
        </nav>
        <div className="border-t-2 border-ink p-3">
          {user && (
            <div className="flex items-center gap-2.5 rounded-md px-2 py-2">
              {user.avatarUrl ? (
                <img src={user.avatarUrl} alt="" className="h-9 w-9 rounded-full border-chunky border-ink" />
              ) : (
                <div className="flex h-9 w-9 items-center justify-center rounded-full border-chunky border-ink bg-secondary text-sm font-black text-ink">
                  {user.displayName?.[0] ?? user.username?.[0]?.toUpperCase()}
                </div>
              )}
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-bold text-ink">{user.displayName}</div>
                <div className="flex items-center gap-1 text-[11px] font-semibold text-ink-muted">
                  {user.provider === 'GITHUB' && <Github className="h-3 w-3" />}
                  {user.provider}
                </div>
              </div>
              <button
                title="退出登录"
                className="rounded p-1.5 text-ink-muted hover:bg-error hover:text-white"
                onClick={() => {
                  api.logout().catch(() => {});
                  setToken(null);
                  window.dispatchEvent(new Event('cg_auth'));
                  window.location.hash = '#/login';
                }}
              >
                <LogOut className="h-4 w-4" />
              </button>
            </div>
          )}
        </div>
      </aside>
      <main className="ml-60 flex-1 px-8 py-6">{children}</main>
    </div>
  );
}
