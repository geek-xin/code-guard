...
# 产物
target/codeguard-0.2.0.jar          # 可执行 Spring Boot jar
target/codeguard-0.2.0.tar.gz       # Linux/macOS 发布包
target/codeguard-0.2.0.zip          # Windows 发布包

# 运行发布包（Linux/macOS）
tar -xzf target/codeguard-0.2.0.tar.gz
cd codeguard-0.2.0
./run.sh        # 启动（日志在 logs/）
./stop.sh       # 停止

# Windows
# 解压 target/codeguard-0.2.0.zip 后执行 run.bat / stop.bat
```
