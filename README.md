...
<p align="center">
  ...
  <img alt="Release" src="https://img.shields.io/badge/Release-0.2.1-111827?style=flat-square">
...
| 项目接入 | GitHub 仓库 / GitLab 仓库（HTTPS/HTTP 克隆，支持远端分支选择拉取；私有仓库访问令牌在「设置 → Git 访问令牌」统一配置，项目自动复用）/ 本地目录 |
...
| 安全设计 | 密码 PBKDF2 哈希存储、Token 不落前端、Git 访问令牌在「设置」统一配置且接口不返回明文 |
...
# 产物
target/codeguard-0.2.1.jar          # 可执行 Spring Boot jar
target/codeguard-0.2.1.tar.gz       # Linux/macOS 发布包
target/codeguard-0.2.1.zip          # Windows 发布包

# 运行发布包（Linux/macOS）
tar -xzf target/codeguard-0.2.1.tar.gz
cd codeguard-0.2.1
./run.sh        # 启动（日志在 logs/）
./stop.sh       # 停止

# Windows
# 解压 target/codeguard-0.2.1.zip 后执行 run.bat / stop.bat
```
