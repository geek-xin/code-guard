    ...
    public byte[] build(ScanRecord scan, List<ScanFinding> findings) {
        try {
            ...
            for (ScanFinding f : sorted) {
                ...
                table.addCell(cell(safe(f.getTitle()), bodyFont, false));
                table.addCell(cell(engineLabel(f.getEngine()), bodyFont, false));
                table.addCell(cell(safe(f.getFile()) + (f.getLine() != null ? " : " + f.getLine() : ""), bodyFont, false));
                table.addCell(cell(safe(f.getVulnId()), bodyFont, false));
            }
            doc.add(table);
            doc.add(new Paragraph(" ", bodyFont));
            ...
            if (scan.getAgentReview() != null && !scan.getAgentReview().isBlank()) {
                ...
                for (String line : scan.getAgentReview().split("\n")) {
                    Paragraph p = new Paragraph(safe(line), bodyFont);
                    p.setSpacingAfter(4);
                    doc.add(p);
                }
            }
